...
function indexPlots(genre, data) {
    ...
    budget = Array.from(budget.entries())
    // budget.toFixed(2)
    // parseFloat(budget.map(d => d[1])).toFixed(2);
    console.log(budget)
    var domestic_gross = d3.rollup(ndata, v => d3.mean(v, d => d.domestic_gross), d => d.year_released);
    domestic_gross = Array.from(domestic_gross.entries())
    var international_gross = d3.rollup(ndata, v => d3.mean(v, d => d.international_gross), d => d.year_released);
    ...
    // console.log(theatres)
    var values = []
    values = [budget.map(d => d[0]) , budget.map(d => parseFloat(d[1]).toFixed(2).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")),
            movie_count.map(d => d[1]) , domestic_gross.map(d => parseFloat(d[1]).toFixed(2).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")) ,
            international_gross.map(d => parseFloat(d[1]).toFixed(2).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")) ,
            world_gross.map(d => parseFloat(d[1]).toFixed(2).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")) ,
            theatres.map(d => d[1])]
    // values = [budget.map(d => d[0]) , budget.map(d => d[1]) , movie_count.map(d => d[1]) , theatres.map(d => d[1])]
    console.log(values)
    //Worls vs Domestic and International Time Series
    ...
    var data = [{
        type: 'table',
        header: {
            values: [["<b>Year</b>"], ["<b>Avg. Budget</b>"], ["<b>Movies</b>"], 
            ["<b>Avg. Domestic Gross Income</b>"], 
            ["<b>Avg. International Gross Income</b>"], ["<b>Avg. World Gross Income</b>"], 
            ["<b>Total Theatre Releases</b>"]],
            align: "center",
            line: { width: 1, color: '#506784' },
            fill: { color: '#119DFF' },
            font: { family: "Arial", size: 12, color: "white" }
...
function init() {
    ...
    d3.json("/api/v1/resources/movies").then(function(data) {
        ...
        var options = selObj.selectAll("option")
            ...
            .attr('value', (v => v))
            .text((t => t));
        // Fetch genre for the default plot
        var genre = '';
        // Call function to build scatter plots
        indexPlots(genre, data);
        // heat_map(json);
